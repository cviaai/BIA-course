...
def imfilter_transpose2D_SpatialDomain(input,kernel,padType="symmetric",mode="conv"):
    ...
    #if channels != 1:
    #    kernel = kernel.expand(-1,channels,-1,-1)
...
#    out = th.conv_transpose2d(input,kernel,groups = groups)
    ...
    kernel = kernel[:,None]
    out = th.conv_transpose2d(input, kernel, groups=kernel.shape[0])
...
#        groups = channels
    ...
    kernel = kernel.reshape(kernel.shape[0]*kernel.shape[1], kernel.shape[2], kernel.shape[3])
    kernel = kernel[:,None]
    out = th.conv2d(input, kernel, groups=kernel.shape[0])
    out = out[0].reshape(b, c, out.shape[2], out.shape[3])
#    out = th.conv2d(input,kernel,groups = groups)
